const options = {
  ...
  botao?.addEventListener("click", event =>{
    ...
    let url = `https://api.themoviedb.org/3/tv/popular?language=pt-BR&page=${page}`;
      ...
      for(let i = 0; i <= data.results.length;i++){
          if(data?.results[i]){
              div.innerHTML += `<div><img id = 'capa' src = "http://image.tmdb.org/t/p/original${data.results[i].poster_path}" height = "320" width = "250"/>
                            <div class = "topicos"><p>${data.results[i].original_name} (${data.results[i].name})</p>
                            <p>${data.results[i].first_air_date.slice(0, 4)}</p>
